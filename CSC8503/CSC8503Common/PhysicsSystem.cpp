...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -25.8f, 0.0f));
}
...
rocket launcher, gaining a point when the this hits the gold coin, and so on).
*/
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if      (physA->name == "PUSHTILE")      { physB->ApplyLinearImpulse(Vector3(0, 60,-20)); }
	else if (physB->name == "PUSHTILE")      { physA->ApplyLinearImpulse(Vector3(0, 60,-20)); }
	
	if (physA->name == "MOTORTILE") { physB->ApplyLinearImpulse(Vector3(0, 0, -50)); }
	else if (physB->name == "MOTORTILE") { physA->ApplyLinearImpulse(Vector3(0, 0, -50)); }

	if (physA->name == "PLAYER" && physB->name == "BONUS") { b.playerCollision = true; b.isAlive = false; }
	else if (physA->name == "BONUS" && physB->name == "PLAYER") { a.playerCollision = true; a.isAlive = false; }

	if (physA->name == "ENEMY" && physB->name == "BONUS") { b.isAlive = false; }
	else if (physA->name == "BONUS" && physB->name == "ENEMY") { a.isAlive = false; }

	if (physA->name == "PLAYER" && b.isLast == true) { b.finished = true; b.playerCollision = true; }
	else if (physB->name == "PLAYER" && a.isLast == true) { a.finished = true; a.playerCollision = true; }


	if (physA->name == "ENEMY" && b.isLast == true) { b.finished = true; b.playerCollision = false; }
	else if (physB->name == "ENEMY" && a.isLast == true) { a.finished = true; a.playerCollision = false; }


	if (a.needsImpulse == true && b.needsImpulse == true) {
...
void PhysicsSystem::BroadPhase() {
	...
	gameWorld.GetObjectIterators(first, last);
	for (auto i = first; i != last; ++i) {
		if ((*i)->GetPhysicsObject()->name == "PLAYER") {(*i)->GetPhysicsObject()->collidingWithFloor = false; }
		if ((*i)->GetPhysicsObject()->name == "ENEMY") { (*i)->GetPhysicsObject()->collidingWithFloor = false; }
		Vector3  halfSizes;
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		linearVel.z *= (*i)->GetPhysicsObject()->friction;
		linearVel.x *= (*i)->GetPhysicsObject()->friction;
		object->SetLinearVelocity(linearVel);

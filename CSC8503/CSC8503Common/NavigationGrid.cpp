		...
		for (int x = 0; x < gridWidth; ++x) {
			...
			for (int i = 0; i < 4; ++i) {
				if (n.connected[i]) {
					...
					if (n.connected[i]->type == 'm' || n.connected[i]->type == 'i') {
						n.costs[i] = 1;
					}
					if (n.connected[i]->type == 'c') {
						n.costs[i] = -5;
					}
					if (n.connected[i]->type == 'y' || n.connected[i]->type == 'o' || n.connected[i]->type == 'j') {
						n.costs[i] = -15;
					}
					if (n.connected[i]->type == 'x' || n.connected[i]->type == 'l' || n.connected[i]->type == 'r') {
